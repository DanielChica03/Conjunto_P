...
class pagoscontroller extends Controller
{
    ...
    public function index()
    {
        ...
        if ($usuario && strtoupper($usuario->tipo_usuario) === 'CLIENTE') {
            // Solo pagos del cliente logueado (filtrado en la consulta)
            $pagos = \App\Models\pagos::whereHas('deuda.venta', function($q) use ($usuario) {
                    $q->where('cliente_id', $usuario->cedula);
                })
                ->with('deuda.venta')
                ->latest()
                ->get();
